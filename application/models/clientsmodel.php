<?php
	...
	{	
		...
		public function generateapplicantionnumber(){			
			$sqlstr	= "SELECT applicant_id FROM applicant ORDER BY applicant_id DESC LIMIT 1";
			$query = $this->db->query($sqlstr);
			if ($query->num_rows() > 0){
				// $row = $query->result();
				// return $recordno = $query->num_rows();			
				return $recordno =  $query->row(0)->applicant_id;				
			}else{
				return 0;
			}
		}
		...
		public function getState(){
			$sqlstr	= "SELECT DISTINCT statename, state_id FROM states ORDER BY statename ASC";
			$query = $this->db->query($sqlstr);
			if ($query->num_rows() > 0){
				$select = "<input type='hidden' class='form-control' id='otherstatesTag' name='otherstatesTag' value='F'>";
				// $select .= "<input type='hidden' class='form-control' id='otherlgasTag' name='otherlgasTag' value='F'>";
				$select .= "<select name='states' id='states' class='required form-control' style='height:50px;' onchange='getLGAs(this.value)'>";
					$select .= "<option value='--'>Select State</option>";
					foreach($query->result() as $row){
						$select .= "<option value='" . $row->statename . "'>$row->statename</option>";
					}
				$select .= "</select>";
			}else{
				$select = "";
		...
		public function getLGAs(){
			$sentState = $_POST['sentState'];
			$sqlstr	= "SELECT lg_id, lg FROM states WHERE statename='$sentState' ORDER BY lg_id ASC";
			$query = $this->db->query($sqlstr);
			if ($query->num_rows() > 0){
				$select = "<input type='hidden' class='form-control' id='otherlgasTag' name='otherlgasTag' value='F'>";
				$select .= "<select name='lgas' id='lgas' class='required form-control' style='height:50px;'>";
					$select .= "<option value='--'>Select LGA</option>";
					foreach($query->result() as $row){
						$select .= "<option value='" . $row->lg . "'>$row->lg</option>";
					}
				$select .= "</select>";
			}else{
				$select = "";
		...
		public function submitapplication(){
			...
			// 	$applicant_image_hidden_tag = "T";
			// }else{
			// 	$applicant_image_hidden_tag = "F";
			// }

			// if($parent_1_image_hidden_tag == "T"){
			// 	$parent_1_image_hidden_tag = "T";
			// }else{
			// 	$parent_1_image_hidden_tag = "F";
			// }

			// if($parent_2_image_hidden_tag == "T"){
			// 	$parent_2_image_hidden_tag = "T";
			// }else{
			// 	$parent_2_image_hidden_tag = "F";
			// }

			// if($applicant_image_hidden_tag == "T" && $parent_1_image_hidden_tag == "T" && $parent_2_image_hidden_tag == "T"){
				$application_number = trim($application_number);
 
			// }else if($applicant_image_hidden_tag == "T" $parent_1_image_hidden_tag == "T"){

			// }
			
			$physical_challenge_details  =  str_replace("'", "", $physical_challenge_details);
			$physical_challenge_details  =  trim(stripslashes($physical_challenge_details));
			
			$medical_condition_details  =  str_replace("'", "", $medical_condition_details);
			$medical_condition_details  =  trim(stripslashes($medical_condition_details));


			$sqlstr = "SELECT * FROM applicant WHERE application_number = '$application_number'";
			$query = $this->db->query($sqlstr);
			if ($query->num_rows() > 0){
				//Data without the picture tags
				$DataArrayX 	= array( 
					'application_number'   => $application_number,
					'applicant_surname'     => trim(ucwords($surname)),
					'applicant_firstname'     => trim(ucwords($firstname)),          		
					'applicant_middlename'     => trim(ucwords($middlename)),          		
					'applicant_religion'     => $religion,          		
					'applicant_address'     => trim($address),          			
					'applicant_country_of_birth'     => trim($country_of_birth),          	
					'applicant_hometown'     => trim($hometown),          			
					'applicant_nationality'     => $nationality,          		
					'applicant_states'     => trim($states),          			
					'applicant_lgas'     => trim($lgas),          				
					'applicant_DOB'     => $DOB,          				
					'applicant_gender'     => $gender,          				
					'applicant_bloodgroup'     => $bloodgroup,          		
					'applicant_genotype'     => $genotype,          			
					'applicant_allergy'     => trim($allergy),          			
					'applicant_parent_surname_1'     => trim($parent_surname_1),          	
					'applicant_othernames_1'     => trim($othernames_1),          		
					'applicant_occupation_1'     => trim($occupation_1),          		
					'applicant_relationship_to_child_1'     => trim($relationship_to_child_1),    
					'applicant_address_1'     => trim($address_1),          		
					'applicant_phone_number_1_1'     => trim($phone_number_1_1),          	
					'applicant_phone_number_1_2'     => trim($phone_number_1_2),          	
					'applicant_email_1'     => trim($email_1),          			
					'applicant_parent_surname_2'     => trim($parent_surname_2),          	
					'applicant_othernames_2'     => trim($othernames_2),          		
					'applicant_occupation_2'     => trim($occupation_2),          		
					'applicant_relationship_to_child_2'     => trim($relationship_to_child_2),    
					'applicant_address_2'     => trim($address_2),          		
					'applicant_phone_number_2_1'     => trim($phone_number_2_1),          	
					'applicant_phone_number_2_2'     => trim($phone_number_2_2),          	
					'applicant_email_2'     => trim($email_2),          			
					'applicant_physical_challenge'     => $physical_challenge_details,  
					'applicant_medical_condition'     => $medical_condition_details,  
					'applicant_fullname'     => trim($fullname),          			
					'applicant_phone_number'     => trim($phone_number),          		
					'applicant_relationship_to_child_3'     => trim($relationship_to_child_3),    
					'applicant_address_3'     => trim($address_3)       
				);  				
				// update applicant table
				if($this->db->where(array('application_number'=>$application_number))->update('applicant',$DataArrayX)){
					//get picture update status and update accordingly
					$sqlstr = "SELECT applicant_image_hidden_tag, applicant_parent_1_image_hidden_tag, applicant_parent_2_image_hidden_tag FROM applicant WHERE application_number = '$application_number'";
					$query = $this->db->query($sqlstr);
					if ($query->num_rows() > 0){
						$row = $query->result();
						$applicant_image_hidden_tag_DB = $query->row(0)->applicant_image_hidden_tag;
						$applicant_parent_1_image_hidden_tag_DB = $query->row(0)->applicant_parent_1_image_hidden_tag;
						$applicant_parent_2_image_hidden_tag_DB = $query->row(0)->applicant_parent_2_image_hidden_tag;
						
						if($applicant_image_hidden_tag_DB == "T")	{
							// Do nothing
						}else{
							if($applicant_image_hidden_tag == "T"){
								$sqlstr = "UPDATE applicant SET applicant_image_hidden_tag='T'
								WHERE application_number = '$application_number'";
								$query = $this->db->query($sqlstr);
							}
						}

						if($applicant_parent_1_image_hidden_tag_DB == "T")	{
							// Do nothing
						}else{
							if($applicant_parent_1_image_hidden_tag == "T"){
								$sqlstr = "UPDATE applicant SET applicant_parent_1_image_hidden_tag='T'
								WHERE application_number = '$application_number'";
								$query = $this->db->query($sqlstr);
							}
						}

						if($applicant_parent_2_image_hidden_tag_DB == "T")	{
							// Do nothing
						}else{
							if($applicant_parent_2_image_hidden_tag == "T"){
								$sqlstr = "UPDATE applicant SET applicant_parent_2_image_hidden_tag='T'
								WHERE application_number = '$application_number'";
								$query = $this->db->query($sqlstr);
							}
						}
					...
					$sqlstr = "SELECT applicant_id, application_number FROM applicant WHERE application_number = '$application_number'";
					$query = $this->db->query($sqlstr);
					if ($query->num_rows() > 0){
						$row = $query->result();
						$_SESSION['applicant_picture'] = $query->row(0)->application_number . ".jpg";
						$_SESSION['parent1_picture'] = $query->row(0)->application_number . ".jpg";
						$_SESSION['parent2_picture'] = $query->row(0)->application_number . ".jpg";
						return 2;
					}else{
						return 0;
					}	
				...
				$DataArray 	= array( 
					'applicant_image_hidden_tag'   => $applicant_image_hidden_tag,
					'application_number'   => $application_number,
					'applicant_surname'     => trim(ucwords($surname)),
					'applicant_firstname'     => trim(ucwords($firstname)),          		
					'applicant_middlename'     => trim(ucwords($middlename)),       		
					...
					'applicant_phone_number_1_2'     => trim($phone_number_1_2),          	
					'applicant_email_1'     => trim($email_1),          			
					'applicant_parent_1_image_hidden_tag'     => $parent_1_image_hidden_tag,  
					'applicant_parent_surname_2'     => trim($parent_surname_2),          	
					'applicant_othernames_2'     => trim($othernames_2),          		
					'applicant_occupation_2'     => trim($occupation_2),          		
					'applicant_relationship_to_child_2'     => trim($relationship_to_child_2),    
					...
					'applicant_phone_number_2_2'     => trim($phone_number_2_2),          	
					'applicant_email_2'     => trim($email_2),          			
					'applicant_parent_2_image_hidden_tag'     => $parent_2_image_hidden_tag,  
					'applicant_physical_challenge'     => trim($physical_challenge_details),  
					'applicant_medical_condition'     => trim($medical_condition_details),  
					'applicant_fullname'     => trim($fullname),          			
					'applicant_phone_number'     => trim($phone_number),          		
					'applicant_relationship_to_child_3'     => trim($relationship_to_child_3),    
					'applicant_address_3'     => trim($address_3)       
				);  				
				if($this->db->insert('applicant',$DataArray)){
					$sqlstr = "SELECT applicant_id, application_number FROM applicant WHERE application_number = '$application_number'";
					$query = $this->db->query($sqlstr);
					if ($query->num_rows() > 0){
						$row = $query->result();
		...
		public function subscribe(){
			$email = $_POST['email'];
			$sqlstr	= "SELECT * FROM subscriber WHERE subscriber_email='$email'";
			$query = $this->db->query($sqlstr);
			if ($query->num_rows() > 0){
				return 2;
			}else{
				...
				if($this->db->insert('subscriber',$DataArray)){
					return 1;
				}else{
					return 0;
				}				
		...
		public function joinevent(){
			...
			$sqlstr	= "SELECT * FROM event_prospects WHERE event_prospects_email='$email'";
			$query = $this->db->query($sqlstr);
			if ($query->num_rows() > 0){
				$DataArray 	= array( 
					'event_prospects_comments'   => $comment,
					'event_prospects_fullname'   => trim(ucwords($fullname)),
					// 'event_prospects_email'     => trim($email),
					'event_prospects_url'     => trim($url)
				); 				
				if($this->db->where(array('event_prospects_email'=>$email))->update('event_prospects',$DataArray)){
					return 2;
				}else{
					return 0;
				}				
				...
				if($this->db->insert('event_prospects',$DataArray)){
					return 1;
				}else{
					return 0;
				}				
		...
		public function contactus(){
			...
			$DataArray 	= array( 
				'contact_us_name'   => trim(ucwords($name)),
				'contact_us_email'   => trim($email),
				'contact_us_subject'   => trim($subject),
				'contact_us_comments'   => trim($comments)
			); 
			if($this->db->insert('contact_us',$DataArray)){
				return 1;
			}else{
				return 0;
			}	
		...
		public function register(){
			...
			$DataArray 	= array(
				...
				'users_account_firstname'			=> $this->refineInput($firstname),
				'users_account_lastname'			=> $this->refineInput($lastname),
		...
		public function admincreateaccount(){
			...
			if($CountArray>=2){		
				$DataArray 	= array(
					...
					'users_account_sex'					=> $gender,
					'users_account_phone_number'		=> $this->refineInput($phone_number),
					'users_account_address'				=> $this->refineInput($address),
					...
					'users_account_lga'					=> $this->refineInput($lga)
		...
		public function cashierreinbursesystem(){
			...
			$users_unique_number = $_SESSION['userinformation']->users_unique_number;
			$CountArray = count(explode(".",$_FILES["userfile"]["name"]));
			if($CountArray>=2){
				$DataArray 	= array(
					'cashier_payment_user_unique_number'			=> trim($users_unique_number),
					'cashier_payment_transaction_id'			=> trim($PTI),
					// 'cashier_payment_puc'			=> trim($PUC),
					'cashier_payment_narration'		=> trim($payment_narrattion),
					// 'cashier_payment_timestamp'		=> trim($payment_narrattion),
					'cashier_payment_receipts_tag'	=> 'T'
				);	
			}else{
				$DataArray 	= array(
					'cashier_payment_user_unique_number'			=> trim($users_unique_number),
			...
			$query = $this->db->get_where('cashier_payment', array('cashier_payment_transaction_id'=>$PTI));
			if ($query->num_rows() > 0){
				// $userId =  $query->row(0)->users_account_id;
				if($this->db->where(array('cashier_payment_transaction_id'=>$PTI))->update('cashier_payment',$DataArray)){
					$_SESSION['cashier_payment_receipts'] = $PTI . ".jpg";
					//get user updated information
					// $query1 = $this->db->get_where('users_account', array('email'=>$email));
					// if ($query1->num_rows() == 1){
					// 	$_SESSION['userinformation'] = '';
				...
				if($this->db->insert('cashier_payment',$DataArray)){
					$_SESSION['cashier_payment_receipts'] = $PTI . ".jpg";
					return 1;	
				}else{
					return 0;
		...
		public function getreceipts(){
			$sentReceiptsID = $_POST['sentReceiptsID'];
			$sqlstr	= "SELECT * FROM cashier_payment WHERE cashier_payment_transaction_id='$sentReceiptsID'";
			$query = $this->db->query($sqlstr);
			if ($query->num_rows() > 0){
	            $row = $query->result();
	            return json_encode($row);
		...
		public function createcontributionbasedcustomeraccount(){
			...
			if($customer_picture_hidden_tag == "T"){
				$DataArray 	= array(
					...
					'customer_bank_account_number'						=> trim($contributor_account_number),
					'customer_account_bvn'								=> trim($contributor_bvn),
					'customer_bank_name'								=> ucwords(trim($contributor_bank_name)),
					'customer_bank_account_name'						=> ucwords(trim($contributor_bank_account_name)),
		...
		public function populateusersdatatable(){						
			$query = $this->db->order_by('users_account','ASC')->get_where('users_account_id');
	        $sOutput = '{';
	        $sOutput .= '"aaData": [ ';       	         
	        if($query->num_rows() > 0){
	            foreach($query->result() as $row){
	            ...
	            foreach($mdata as $tr) {
					$noo++;
					$fullname = $tr->users_account_firstname.' '.$tr->users_account_lastname;
					$button = "<button type='button' class='btn green' value='".$tr->users_account_id."' onclick='editrec(this.value)'>Edit</button>";				
	                $sOutput .= "[";
	                $sOutput .= '"'.str_replace('"', '\"', $noo).'",';
	                $sOutput .= '"'.str_replace('"', '\"', $fullname).'",';
	                $sOutput .= '"'.$button.'",';
			...
			if ($query->num_rows() > 0 && $accountingyear != ""){
				...
				if ($query->num_rows() > 0){
					...
					for($i = 0; $i < count($result); $i++){
						$contribution_unique_ids = $result[$i]->dialy_contribution_year_id.'~'. $result[$i]->customer_contribution_id;
						$recordNo++;
						$btn = "<button type='button' class='btn btn-primary' id='".$contribution_unique_ids."' onclick='previewContributorByContributorYearId(this.id)'>Preview</button>";
						$tr .= "<tr>";
		...
		public function dailycontributioncard(){
			$sendContributorUniqueIds = $_POST['sendContributorUniqueIds'];
			$sendContributorUniqueIds = explode('~', $sendContributorUniqueIds);
			$sendDailyContributionYearId = $sendContributorUniqueIds[0];
			$sendCustomerContributionId = $sendContributorUniqueIds[1];
			// echo $sendCustomerContributionId.' '.$sendDailyContributionYearId;
			// exit;
			//debtors
			//get the transaction year that is the one that is activated
			$sqlstr	= "SELECT * FROM accounting_year WHERE accounting_year_status='Activated'";
			...
			if ($query->num_rows() > 0 && $accountingyear != ""){
				...
				$sqlstr1	.= "&& $transactionYear.customer_contribution_id = '$sendCustomerContributionId' ";
		...
		public function processrepaymment(){
			extract($_POST);
			//get the transaction year that is the one that is activated
			// echo 'agreement_id='.$agreement_id.'repayment='.$repayment.'savings='.$savings.'accountnumber='.$accountnumber.'day='.$day.'week='.$week;
			// exit;
			...
			if ($query->num_rows() > 0 && $accountingyear != ""){
				...
				if ($query->num_rows() > 0){
					...
					// echo 'dayid='.$dayid.'dayname='.$dayname.'dbweek'.$dbweek;
					// exit;
